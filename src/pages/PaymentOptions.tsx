        ...
        <div className="w-full max-w-6xl mx-auto p-4 space-y-6">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {paymentPlans.map((plan, index) => (
              <Card 
                key={plan.id}
                className={`relative cursor-pointer transition-all duration-200 hover:shadow-lg animate-slide-up ${
                  ...
                  <ul className="space-y-2 mb-6">
                    {plan.features.map((feature, idx) => (
                      <li key={idx} className="flex items-center gap-2 text-sm">
                        ...
                        <span className={feature.includes('Pay in advance') ? 'text-primary font-medium' : ''}>
                          {feature}
                        </span>
                      </li>
                    ))}
              ...
              {selectedPlan && (
                <div className="space-y-3">
                  ...
                  <div className="flex justify-between text-lg font-bold">
                    <span>Total:</span>
                    <span className="text-primary">₦{paymentPlans.find(p => p.id === selectedPlan)?.amount.toLocaleString()}</span>
                  </div>
                </div>
              )}
            </CardContent>
